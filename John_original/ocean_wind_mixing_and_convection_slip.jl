...
λ = 2;      # decay length of slip velocity at z=0 (m)
...
for k=0:Nz+2
  slip_vel.w[:,:,k].+=ws*tanh(max(-grid.zᵃᵃᶠ[k]/λ,0));
end
...
anim = @animate for (i, iter) in enumerate(iterations[intro:end])
    ...
    c_plot = contourf(xT, zT, c'; color=:haline, clims=clims, levels=clevels, kwargs...)
    ...
    plot(w_plot, T_plot, c_plot, ν_plot, layout=(2, 2), size=(1200, 600),
         title=[w_title T_title c_title ν_title])
